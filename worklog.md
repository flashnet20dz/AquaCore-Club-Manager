...
- تشغيل الخادم والتحقق بالمتصفح: تسجيل دخول admin@example.com ✓، كل التبويبات الـ23 ✓، تسجيل حضور تفاعلي (POST 201) ✓، لوحة مالية مطابقة للبيانات ✓، تنبيهات تجديد (6) ✓، تجاوب موبايل 390px ✓، كونسول نظيف ✓
Stage Summary:
...
- الحسابات: admin@example.com/********، coach@example.com/********، guard@rcs.dz/********، PIN 1234
---
Task ID: sec-fixes-1
...
- نسخ المستودع إلى /home/z/AquaCore-Club-Manager وكتابة 4 إصلاحات أمنية: قفل cron بـ CRON_SECRET (timing-safe)، إزالة باكدور ******** (SEED_DEFAULT_ADMIN + كلمة سر عشوائية)، whitelist أدوار في POST /api/users، عزل sync/push (قائمة نماذج + فحص ملكية clubId + فرض clubId + حظر delete غير المدعوم)
...
- تحقق متصفح: دخول admin@example.com ✓ لوحة التحكم بـ42 منخرطاً ✓ صفر أخطاء كونسول ✓
Stage Summary:
...
- متطلبات ما بعد التطبيق: ضبط CRON_SECRET في البيئة وترويسة x-cron-secret بجدولة Vercel، تدوير مفتاح Cloudinary المسرب، تغيير كلمات سر ******** في أي نشر قائم
...
- تحقق من DB المحلي: admin@example.com نشط و bcrypt.compare('********') ✓ → البيانات صحيحة والمشكلة في الكوكي
...
- ملاحظة: admin@example.com على الإنتاج superadmin بلا نادي — فحص APIs النادي العميقة تم محلياً وعلى حساب النادي التجريبي؛ نمط أعمدة runtime-self-heal مثبت من المرحلتين 1-4 يضيف أعمدة الإنتاج تلقائياً عند أول طلب نادي
...
- تحقق Agent Browser (دخول admin@example.com):
...
Task ID: wh-cancel-totals
Agent: main
Task: URGENT BUG — CANCELLED WORK HOURS STILL INCLUDED IN TOTALS (إلغاء سجل لا ينعكس على الإجمالي أسفل جدول الأجور: 14س/5600 → إلغاء 400 → يجب 13س/5200)

Work Log:
- فحص شامل قبل التعديل (بند 1/17 من التدقيق): مسح كل مواضع تجميع WorkHours في المصدر — wage-core (status:"approved" ✓) + /api/stats (approved ✓) + /api/employees/[id]/profile (approved ✓) + staff-compensations/auto-calc (approved+pending ✓) + work-hours-panel (approved ✓) + financial-reports (approved ✓) — الجذر الوحيد: work-hours-management.tsx كان يحسب (stats + staffSummary + تذييل الجدول) من القائمة المعروضة بفلتر حضور فقط (workStatus==="present" من note JSON) بلا أي فحص لحالة دورة الحياة status — فالملغى/المرفوض ظل يدخل في الساعات والأجور
- القاعدة الموحّدة المُقرّرة (بند 3): pending/approved (نشط) يدخل في كل الحسابات التشغيلية — rejected/cancelled لا يدخل إطلاقاً (يبقى معروضاً بشارة «ملغى» للتاريخ والتدقيق فقط) — «الكل» تعني عرض كل السجلات لا جمع الملغى كأجور مستحقة
- GET /api/workhours: ملخص شهري يُحسب في الخادم (summary.perUser + totals) بفلتر DB صريح status notIn(rejected,cancelled) — الاستبعاد في الاستعلام لا بإخفاء واجهة (بند 4) — الأجر من لقطة السعر لكل سجل (§23 — نفس قاعدة wage-core حتى تبقى صفحة الساعات وصفحة الأجور برقم واحد) + نفس دلالات التقريب السابقة حرفياً
- work-hours-management.tsx: staffSummary والتذييل وبطاقات الإحصاء تستهلك ملخص الخادم (مصدر واحد §27) + احتياط يدوي بنفس القاعدة (isActiveRecord) لاستجابات قديمة بلا ملخص — بطلان تعديل عرض فقط: الأرقام مصدرها الاستعلام
- PATCH /api/workhours/[id]: المعاملة الذرّية runTx (إصلاح P2028 نفسه) مع إعادة قراءة + إعادة فحص داخلها — الإلغاء المزدوج المتزامن (نقر مزدوج) ينتهي بواحد 200 وواحد 409 حتماً (بند 12) + التدقيق داخل المعاملة
- ★ حماية الأجر المدفوع (بند 14 — paidWageGuard): السجل المعتمد الذي يقع تاريخه داخل فترة WagePayment نشطة لنفس العامل يُمنع من الإلغاء/الرفض بـ409 يوجّه لإلغاء التسديد أولاً عبر آلية العكس الرقابية الموجودة (DELETE /api/wages/[id] — Void ناعم يلغي القيد المالي 1:1 ويعيد الرصيد) — ثم ينجح الإلغاء. لا مساس بالتاريخ المالي المدفوع، ولا قيد مالي جديداً عند إلغاء الساعات (Δ الدفتر = 0 مثبتة بالاختبار) — الفحص استباقي + داخل المعاملة (سباق تسديد×إلغاء)
- POST /api/workhours/approve: نفس الحماية على الرفض الجماعي للسجلات المعتمدة داخل فترات مسدَّدة (skip بسبب واضح)
- سكربت الانحدار scripts/workhours-cancel-totals-test.mjs (38 فحصاً): نفس أرقام البلاغ 14 حصة → ملخص 14.0س/5,600دج بالضبط → إلغاء 1س/400 → 13.0س/5,200دج بالضبط + الملغى يبقى في القائمة (بلا حذف) بـcancelledAt/cancelledById + إلغاء مزدوج 409 + سباق متزامن (200×1/409×1 بلا انعكاس مزدوج) + paidWageGuard يمنع إلغاء المسدد 409 + عكس التسديد ثم الإلغاء ينجح + Δ الدفتر = 0 قبل/بعد + تنظيف كامل
- إصلاح هشاشة اختبارات ما قبل الإنتاج (بقايا تشغيلات سابقة كانت تلوّث — ليس تراجعاً من الكود): phase4 اكتسى تنظيفاً مسبقاً ناعماً (نفس نهج phase5) + تنظيفه النهائي أصبح إلغاءً ناعماً ثم حذف مسودات + فحص timezone أصبح نشطاً-فقط + فحص الأجر من اللقطة (§23) — phase5 اكتسى تنظيف بقايا بلا وسم ليوم الاختبار المشترك + سجل تجاوز المدير §25 نُقل 09:00→06:00 (كان يصطدم بحماية الأجر المدفوع الجديدة ثم بحصص timezone عند workerB=workerA محلياً)
- الاختبارات: cancel-totals 38/38 + bulk 22/22 + financial-audit 31/31 + phase4 55/55 + phase5 85/85 + stress تحت الحمل 6/6 (52 قراءة خلفية، صفر P2028) + lint نظيف + build ✓ 100/100
- تحقق متصفح كامل (agent-browser): تسجيل 3 حصص → الملخص والتذييل والبطاقات 3.0س/1,200دج → إلغاء سجل من الواجهة (نافذة السبب) → فوراً 2.0س/800دج في الصف والتذييل والبطاقات + توست «يبقى في السجل بوضع ملغى» + الملغى ظاهر بالقائمة بشارة «ملغى» + موبايل 390px بلا overflow + كونسول نظيف

Stage Summary:
- ROOT CAUSE: تجميعات الواجهة (stats/staffSummary/تذييل جدول الملخص) في work-hours-management.tsx لم تكن تفحص status السجل (فحص حضور فقط) — الملغى/المرفوض ظلّا يدخلان الساعات والأجور؛ وكل مواضع التجميع الخلفية كانت سليمة أصلاً
- FIX: ملخص شهري من الخادم بفلتر DB (notIn rejected,cancelled) يستهلكه الجدول والتذييل والبطاقات + معاملة ذرّية بإعادة فحص للإلغاء (منع مزدوج) + paidWageGuard يمنع إلغاء/رفض السجل المعتمد داخل فترة مسدَّدة ويوجّه لعكس التسديد أولاً + نفس الحماية للرفض الجماعي
- معمارية الأجر محفوظة: إلغاء الساعات ≠ حركة مالية (Δ=0) — الملغى يبقى للتاريخ بلا حذف — من ألغى/متى/لماذا موثّق — اللقطة السعرية أسبق (§23)
- 38/38 + 22/22 + 31/31 + 55/55 + 85/85 + build ✓ + تحقق متصفح (1200→800 فوراً في الواجهة)

Files created:
- scripts/workhours-cancel-totals-test.mjs

Files modified:
- src/app/api/workhours/route.ts (ملخص الخادم — جراحي)
- src/app/api/workhours/[id]/route.ts (runTx + paidWageGuard — جراحي)
- src/app/api/workhours/approve/route.ts (حماية الرفض الجماعي — جراحي)
- src/components/work-hours-management.tsx (استهلاك الملخص + isActiveRecord — جراحي)
- scripts/phase4-test.mjs (تنظيف مسبق + فحوص نشطة-فقط + لقطة §23)
- scripts/phase5-test.mjs (تنظيف بقايا بلا وسم + §25 06:00)

---
Task ID: wh-cancel-totals-deploy
Agent: main
Task: نشر إصلاح «الملغى يبقى في الإجمالي» على الإنتاج — دمج مع تاريخ الإنتاج المتوازي + Vercel + دخان

Work Log:
- اكتشاف تاريخ متوازٍ على الإنتاج: جلسة نشر موازية دفعت مسبقاً FINAL AUDIT (fc53f0c) + بطاقة المداخيل (4bfaeed) + إصلاح P2028 (a196fed) بأ_SHA مختلفة — المحلي كان خلف الإنتاج بـ24-commit مكررة
- الدمج الآمن (بلا force): فرع من طرف الإنتاج (9cfdc07) + cherry-pick لإصلاح الإلغاء فقط — صراعان حُلّا جراحياً: phase4-test (تنظيف مسبق: دمج نهجي التاريخين — شهر كامل للعاملين + إبطال تسديدات من تشغيلات منهارة، وفحص timezone = موسوم+نشط) و worklog (حفظ الروايتين)
- 231/231 على شجرة الدمج نفسها قبل الدفع: cancel-totals 38/38 + bulk 22/22 + phase4 55/55 + phase5 85/85 + fin-audit 31/31 + lint نظيف
- الدفع: f7e7e98 → main (9cfdc07..f7e7e98) — Vercel: pending → success (~2د)
- دخان الإنتاج (قراءة فقط): home 200 + login 200 + me superadmin بلا نادي (سلوك مثبت) + /api/workhours?month=2026-09 → 200 مع summary.rule=active-only — بيانات النادي الحقيقية: 9 سجلات منها 4 ملغاة تُعرض بلا حساب → الإجمالي 10 ساعات من 5 سجلات نشطة فقط (مطابق لتوقّع البلاغ: 14h الظاهرة سابقاً كانت تجمع الملغى)
- ملاحظة: superadmin بلا نادي يرى totalWage=0 (لا Employee بلا clubId) — نفس سلوك حقن hourlyRate القديم للسوبر أدمن؛ مدير النادي الفعلي يرى الأجور صحيحة

Stage Summary:
- الإنتاج f7e7e98 (Vercel success) — الملخص الشهري النشط-فقط حي على الإنتاج + حماية الأجر المدفوع + منع الإلغاء المزدوج
- المحلي main متزامن مع الإنتاج (f7e7e98) — لا التباس بين النسختين بعد اليوم
---
Task ID: workhours-delete-on-error
Agent: main
Task: أضف ميزة حذف سجل ساعات العمل في حالة الخطأ (Add work-hours record deletion on error)

Work Log:
- فحص كامل دورة حياة سجل WorkHours: schema (status: pending/approved/rejected/cancelled + cancelledById/cancelledAt + rateSnapshot)، و GET /api/workhours (ملخص active-only يستبعد rejected/cancelled من الإجماليات — إصلاح الجلسة السابقة مؤكد)، و DELETE /api/workhours/[id] (كان يحذف المسودات فقط ويرفض البقية بـ 409 requiresCancel)، و wage-core (status:"approved" فقط)، و كل نقاط التجميع الأخرى (stats / financial-dashboard / staff-compensations / employees profile) — كلها تفلتر approved، لا ثغرات.
- أعادت كتابة DELETE /api/workhours/[id] — ميزة «الحذف عند الخطأ»:
  * مسودة (pending): صاحبها أو المدير يحذفها بلا سبب إلزامي (كما كان).
  * معتمد/مرفوض/ملغى: حذف نهائي للمدير/المساعد فقط (workHoursApproval) + سبب إلزامي ≥3 أحرف (من body أو ?reason=).
  * ★ paidWageGuard: السجل المعتمد داخل فترة WagePayment نشطة لا يُحذف (409) — المسار الرقابي: عكس التسديد أولاً ثم الحذف.
  * معاملة ذرّية runTx: إعادة قراءة + حماية + حذف + AuditLog معاً — الحذف المزدوج المتزامن → 200/404.
  * لقطة تدقيق كاملة في metadata: deletedFromStatus، reason، grossAmount، hours، rateSnapshot، الأوقات، من حذف — الحذف الفعلي لا يفقد التاريخ أبداً.
- واجهة work-hours-management.tsx:
  * زر الحذف (Trash2) أصبح يظهر لكل الحالات (كان للمسودات فقط) — title ديناميكي: «حذف المسودة» / «حذف نهائي — عند الخطأ في التسجيل».
  * حوار تأكيد شامل بدل confirm(): بيانات السجل (العامل/التاريخ/الحالة/الساعات/الأجر) + تحذير الحذف النهائي + سبب إلزامي (غير المسودات) + زر معطّل حتى إدخال سبب صحيح + رسائل خطأ الخادم تُعرض (paidWageGuard).
  * بعد الحذف: toast برسالة الخادم + إعادة جلب + تحديث قسم الأجور (wagesRefresh).
- اختبار انحدار جديد scripts/workhours-delete-test.mjs (28/28 ✅): السبب إلزامي (400 بلا سبب/قصير) → حذف بسبب صحيح (200) → السجل يختفي والإجماليات تنخفض فوراً (3.0سا/1200دج → 2.0سا/800دج) → حذف مزدوج 404 → قيد تدقيق work_hour_delete باللقطة الكاملة → الدفتر المالي Δ=0 → paidWageGuard 409 → عكس التسديد ثم الحذف 200 → تنظيف كامل.
- تحديث تنظيف workhours-cancel-totals-test.mjs لاستخدام الحذف الفعلي بسبب (بدل ترك سجلات ملغاة متراكمة) — أُعيد تشغيله: 38/38 ✅ (سيناريو البلاغ 14سا/5600دج → إلغاء → 13سا/5200دج ما زال يمر).
- lint: صفر أخطاء في الملفات المعدّلة (الأخطاء الـ15 كلها موروثة في scripts/ و مكونات أخرى).
- تحقق بالمتصفح (agent-browser): دخول admin → تبويب ساعات العمل → زر «حذف نهائي» يظهر على سجل ملغى → الحوار يفتح بالبيانات والتحذير والسبب الإلزامي والزر معطّل → بعد إدخال السبب يتفعل → الحذف 200 → السجل اختفى من القائمة (3→2) → قيد تدقيق «حذف نهائي لسجل ساعات عمل (ملغى) عند الخطأ» موجود → الحوار يعمل على الجوال (390×844) → dev.log بلا أخطاء.

Stage Summary:
- ✅ ميزة «حذف سجل ساعات العمل عند الخطأ» جاهزة: API + UI + تدقيق كامل + حمايات مالية.
- ✅ قاعدة الذهب: الإلغاء الناعم يبقى متاحاً (يحفظ التاريخ بشارة ملغى)، والحذف النهائي الآن خيار للمدير عند الخطأ الفعلي — كلاهما مستثنى من كل الإجماليات دائماً.
- ✅ حماية الأجر المدفوع تحمي الحذف كما تحمي الإلغاء — لا مساس بالتاريخ المالي المدفوع أبداً.
- ✅ 28/28 اختبار حذف + 38/38 اختبار إلغاء + تحقق متصفح كامل (Desktop + Mobile).
- الملفات: src/app/api/workhours/[id]/route.ts، src/components/work-hours-management.tsx، scripts/workhours-delete-test.mjs (جديد)، scripts/workhours-cancel-totals-test.mjs (تنظيف محسّن).
---
Task ID: login-fix-sandbox-reset
Agent: main
Task: «الحساب لا يعمل» — خطأ داخل في النظام عند تسجيل الدخول (admin@example.com)

Work Log:
- شخّص الجذر: POST /api/auth/login → 500 «Error validating datasource db: the URL must start with the protocol postgresql://» — كتلة datasource في prisma/schema.prisma المحلي تحوّلت إلى postgresql+directUrl (نسخة الإنتاج) بينما .env المحلي DATABASE_URL=file:db/custom.db (SQLite).
- الإصلاح 1 (جراحي — درس b8f4e5d): استعادة provider="sqlite" وإزالة directUrl فقط، مع الإبقاء على حقول StaffCompensation الجديدة (archivedAt/archivedById/archiveReason) لأن الكود المحلي يستخدمها و ensureRuntimeColumns يضيف أعمدتها تلقائياً.
- الإصلاح 2: npx prisma generate → العميل أعيد توليده بـ sqlite.
- الإصلاح 3: إعادة تشغيل خادم التطوير — الاكتشاف المهم: عمليات setsid/nohup المباشرة تُقتل بصمت عند نهاية جلسة الأوامر؛ الطريقة الصحيحة في هذه البيئة: bash .zscripts/dev.sh (المسار الرسمي: bun install + db:push + dev + disown).
- الاكتشاف الثاني: بعد الإقلاع — login 401 «بيانات الدخول غير صحيحة» وجدول User فارغ تماماً: بيئة العمل أُعيد تهيئتها وملف db/custom.db فُقد (db/ في .gitignore — غير مرفوع في git). القاعدة الحالية جديدة فارغة أنشأها db:push.
- الإصلاح 4: سكربت استعادة جديد scripts/recover-admin.mjs (idempotent): نادي نشط (النادي الهاوي متعدد الرياضات — سعيدة، تجربة 15 يوم) + admin@example.com/******** (bcrypt، role=admin، مرتبط بالنادي) + إعدادات أساسية (clubName/currency=دج/workHourRate=400...).
- تحقق: login API → 200 (user object سليم) • دخول عبر الواجهة بالمتصفح → لوحة التحكم تفتح «تجربة — 15 يوم» • كل endpoints تعيد 200 في dev.log • تبويبات كلها حاضرة.

Stage Summary:
- ✅ الدخول يعمل: admin@example.com / ******** (API + UI).
...
- تحقق متصفح كامل للإنتاج (agent-browser): دخول admin@example.com → لوحة السوبر-أدمن تُحمَّل ببيانات حقيقية (نشاطان، نوادٍ نشطة: الفوق/البركة، تنبيهات اشتراكات 7/30 يوم) + صفحة الدخول RTL كاملة بالميزات.
...
---
Task ID: wage-receipt-print
Agent: Z.ai Code (main)
Task: «أريد إضافة وصل استلام عند تسديد أجور العمال في صفحة ساعات العمل لكل عامل باسمه و معلوماته الشخصية عند التسديد تكون هناك طباعة الوصل و يقوم العامل بإمضائه لتسهل العمل» — وصل استلام مستحقات مالية قابل للطباعة عند كل تسديد أجر

Work Log:
- 📄 قرأت قالب الوصل الرسمي المرفوع (docx): ترويسة رسمية 4 أسطر (الجمهورية/الوزارة/المديرية الولائية/النادي) + رقم الوصل والموسم الرياضي + جدول بيانات المستفيد + إقرار بالاستلام + المبلغ بالأرقام وبالحروف + موضوع الدفع + طريقة الدفع + حُرّر بـ... في: + خانتا التوقيع والختم
- 🆕 أنشأت /api/wages/receipt (GET ?id=&legacy=1): بيانات الدفعة + معلومات العامل الشخصية من سجل الموظف (الاسم، الوظيفة مع خرائط المواقع، رقم بطاقة التعريف الوطنية) + اسم النادي ومدينته + رقم وصل تسلسلي (عدّاد زمني سنوي للتسديدات النشطة) + رقم القيد المالي FIN-YYYY-NNNNNN (مولّد من seq بنفس تنسيق postLedgerEntry) — يدعم حتى السجلات القديمة legacy (Payment category=salary) — صلاحية نفس قارئ الأجور
- 🆕 أنشأت src/components/wages/wage-receipt.ts: نافذة طباعة A4 RTL مطابقة للقالب (نفس نمط receipt.ts) — تفقيط المبلغ بالحروف من @/lib/amount-in-words — الموسم الرياضي (شتنبر→غشت) — الأرقام لاتينية fr-DZ LTR — سطر الحساب (ساعات × سعر = إجمالي) — طريقة الدفع موائمة (نقداً من الصندوق / حوالة أو صك + مرجع القيد) — @media print + @page A4
- 🔌 دمجت في wages-section.tsx (يخدم صفحة ساعات العمل والمركز المالي معاً):
  • حوار «تم التسديد بنجاح» يظهر فور التسديد الناجح مع زر «طباعة الوصل» (click = user gesture → لا حجب نوافذ)
  • زر «وصل» في سجل تسديدات الفترة + في كل التسديدات (يدعم legacy) — يختفي للملغاة
- 🐞 إصلاحات أثناء التحقق: تكرار «دج جزائري» في سطر التفقيط (الأداة تتضمنه أصلاً)، حرف Date الإنجليزي الموروث من القالب الأصلي، اتجاه سطر الحساب (direction:ltr كسر ترتيب الكلمات العربية → RTL طبيعي)
- 🌱 ملاحظة بيئة: قاعدة SQLite المحلية كانت فارغة تماماً (سيرفر الجلسة السابقة كان موصولاً بقاعدة أخرى) — زرعت بيانات اختبار دنيا (نادي «الرائد» سعيدة + admin@example.com/******** + موظف حارس سباحة ببطاقة تعريف + 3 حصص معتمدة = 21سا×250 = 5250 دج + اشتراك سنوي نشط) — تسديد اختبار 5250 دج موجود محلياً
- 🌐 تحقق متصفح كامل (المسار الذهبي): تسجيل دخول → ساعات العمل → أجور العمال (5,250 دج) → تسديد الأجر → تأكيد الدفع → حوار النجاح → طباعة الوصل → نافذة جديدة بالوصل الكامل: كل حقول القالب حاضرة (الاسم، حارس سباحة، 109901234567 LTR، المبلغ بالحروف صحيح بلا تكرار، FIN-2026-000001، حُرّر بـسعيدة في: 20/09/2026، التوقيعات) — لقطات كاملة في /tmp — tsc وlint نظيفان، dev.log بلا أخطاء

Stage Summary:
- ✅ كل تسديد أجر ينتج الآن «وصل استلام مستحقات مالية» رسمياً جاهزاً للطباعة وتوقيع العامل — بالاسم ومعلوماته الشخصية (الوظيفة + رقم بطاقة التعريف الوطنية من سجل الموظف) والمبلغ بالأرقام والحروف ورقم وصل تسلسلي ومرجع القيد المالي
- ✅ الوصول للوصل من 3 مواضع: فور التسديد (حوار النجاح) + سجل الفترة + سجل كل التسديدات — والملغاة لا تُطبع
- 📁 ملفات: src/app/api/wages/receipt/route.ts (جديد)، src/components/wages/wage-receipt.ts (جديد)، src/components/wages/wages-section.tsx (حوار النجاح + أزرار الوصل)
- ⚠️ «تاريخ ومكان الصدور» و«رقم الصك» تُملأ يدوياً على الورق إن لم تتوفر في السجلات (متوافق مع القالب الرسمي)
- 🔜 ما زال بالقائمة: WiFi بدون إنترنت، عرض كل الأفواج في شاشة التسجيل، يوم الصيانة المختار، استجابة أيام الفتح، رفع GitHub، الحساب لا يعمل، مهلة التسجيل الجماعي

---
Task ID: latin-dates-ltr
Agent: Z.ai Code (main)
Task: «تغير كل تاريخ مكتوب بهذه الأرقام ٣/١٠/٢٠٢٦ إلى أرقام عادية وجعلها من اليسار إلى اليمين» — (سُجّلت متأخرة: قسمها السابق فُقد بتبديل ملف worklog من جلسة متوازية؛ أعيد توثيقها)

Work Log (ملخص — التنفيذ تم في الجلسة السابقة):
- 🔍 الجذر: رقعة Intl السابقة كانت تقرأ Intl.DateTimeFormat.prototype.format من الـprototype → يرمي "incompatible receiver" في V8 الحديث → الـtry/catch يبتلع → الرقعة ميتة صامتة + علامات U+200F الخفية من ICU تقلب 3/10/2026 بصرياً إلى 2026/10/3 داخل RTL
- ✅ constructor swap (patchIntlFormatterCtor) لـIntl.DateTimeFormat وIntl.NumberFormat عبر Object.defineProperty لكل-instance (format/formatToParts/formatRange/formatRangeToParts) مع تمرير المخرجات عبر toLatinDigits — نفس الرقعة في سكربت layout.tsx الداخلي (خط الدفاع قبل hydration) — member/[token] غُلّف بـtoLatinDigits
- ✅ lang="ar-DZ" لـ35 حقل input[type="date"] في 17 ملفاً (بدون لمس card-designer-pro.tsx) — حقول التاريخ الأصلية تعرض أرقاماً لاتينية D/M/Y
...
- ✅ تحقق ما بعد الدمج: tsc --noEmit نظيف (EXIT 0)؛ متصفح حي: تسجيل دخول admin@example.com → ساعات العمل → أجور العمال — صفر أرقام شرقية، تواريخ لاتينية D/M/Y (14/09/2026...)، زرا «طباعة وصل استلام 5 250 دج» حاضران، النقر يفتح الوصل كاملاً (رقم 1/2026، التفقيط، التوقيعات) — الميزتان تعملان معاً في شجرة واحدة
...
---
Task ID: receipt-worker-info-signatories
Agent: Z.ai Code (main)
Task: ①«عند إنشاء مستخدم جديد أضف تسجيل معلومات حسب وصل الإستلام لتملأ مباشرة عند طباعة الوصل» ②«أضف إختيار من يمضي في آخر الوصل: توقيع واستلام المعني(ة) بالأمر / ختم وتوقيع إدارة النادي (أمين المال / رئيس النادي) / ختم مصالح البلدية» + القالب الرسمي المرفق docx

Work Log:
- 📄 حللت القالب الرسمي المرفق (docx + صورة): جدول المستفيد (الاسم واللقب/الصفة/رقم البطاقة/تاريخ ومكان الصدور) + الإقرار + المبلغ بالأرقام والحروف + الموضوع + الطريقة + خانات التوقيع
- 🗄️ Prisma (جراحة): Employee += nationalIdIssueDate DateTime? + nationalIdIssuePlace String? («★ وصل الاستلام») — db:push آمن (عمودان nullables)
- 🔌 API: POST /api/employees + PATCH /api/employees/[id] يقبلان الحقلين المضبوطين؛ GET /api/wages/receipt يعيدهما (legacy: null)
- 📝 نموذج العامل (contracts-panel.tsx EmployeesTab): حقل «تاريخ صدور البطاقة» (type=date lang=ar-DZ dir=ltr) + «دائرة / بلدية صدور البطاقة» — يُحفظان من إنشاء/تعديل ويُعبّآن مسبقاً عند التعديل
- 🖨️ wage-receipt.ts: صف «تاريخ ومكان الصدور» يُملأ آلياً من ملف العامل (تاريخ لاتيني LTR معزول) ويبقى منقّطاً إن غاب؛ openWageReceiptPrint(data, signatories?) — WageReceiptSignatories {recipient, clubTitle: both|treasurer|president|null, municipality} + DEFAULT_SIGNATORIES؛ خانات التوقيع ديناميكية (2 أو 3 خانات، عناوين النادي حسب الاختيار، fallback إن كُلّ أُخفي)
- 🎛️ wages-section.tsx: حوار «خيارات وصل الاستلام» قبل أي طباعة (من حوار النجاح + من زرّي «وصل» في السجلين): 3 خانات Checkbox + Select لصاحب الصلاحية (أمين المال / رئيس النادي / كلاهما) — زر الطباعة معطّل إن كُلّ الخيارات فارغة
- 🐞 إصلاح انهيار أعاده الدمج: data?.period.label → data?.period?.label + data?.workers.filter → data?.workers?.filter (نسخة origin بلا الحماية)
- 🌱 إعادة زرع بيانات الاختبار: اكتُشفت قاعدة محلية فارغة بعد الدمج (اختفاء صفحات WAL يتيمة بعد قتل خوادم dev المتكررة + إعادة بناء الجداول في db:push) — أُعيد إنشاء: نادي «الرائد» + admin@example.com + عامل ببطاقة (109901234567، صادرة 12/06/2015 دائرة سعيدة) + 21سا×250 + تسديد 5250 دج (FIN-2026-000001)
- ✅ تحقق متصفح ذهبي كامل: تسجيل دخول → عقود العمل → تعديل العامل (الحقول الجديدة حاضرة ومعبّأة) → ساعات العمل → أجور العمال → زر «وصل» → حوار الخيارات (3 خيارات + Select) → اختيار «رئيس النادي فقط» + «ختم مصالح البلدية» → طباعة: الوصل يعرض الصادرة بتاريخ: 12/06/2015 عن دائرة/بلدية: دائرة سعيدة/بلدية سعيدة آلياً + 3 خانات توقيع (المعني + رئيس النادي + البلدية) بلا أمين المال — ثم المسار الافتراضي: خانتان (المعني + أمين المال/رئيس النادي) بلا البلدية — tsc نظيف

Stage Summary:
- ✅ معلومات وصل الاستلام تُسجَّل من ملف العامل عند إنشائه/تعديله وتتدفق آلياً إلى الوصل عند الطباعة (لا تعبئة يدوية على الورق إن سُجّلت)
...
---
Task ID: prod-db-heal-employees-id
Agent: main
Task: إكمال إصلاح الإنتاج — أعمدة وصل الاستلام المفقودة على PostgreSQL + employees/[id] + تحقق إنتاجي شامل

Work Log:
- اختبر الإنتاج بعد النشر: settings تعيد اسم النادي كاملاً ✓ لكن POST employees كشف P2022 (nationalIdIssueDate غير موجود) — الترحيل 20260921100000 وُسم applied دون تنفيذ فعلي
- أضاف العمودين إلى runtime-schema.ts (نمط الذاكِل الذاتي IF NOT EXISTS المثبت) واستدعاه في employees GET/POST
- ترحيل جديد 20260922100000_employee_national_id_issue_columns بإعادة تطبيق آمنة IF NOT EXISTS
- اكتشف أن employees/[id] PATCH+DELETE كانت بنفس علة clubId! — طبّق resolveTargetClubId عليهما
- تحقق إنتاجي بعد النشر: POST employees → 201 (clubId صحيح) → DELETE → 200 (أرشفة نظيفة) → wages → 200 → قالب CDD على الإنتاج v2 بلا سعر الساعة
- نظّف سجل الاختبار من الإنتاج (أرشفة)

Stage Summary:
- ✅ الإنتاج يعمل بالكامل: إضافة/تعديل/أرشفة العمال + الأجور + قالب CDD v2
- ✅ سلسلة الترحيلات أصبحت متسقة (20260922100000 طبّقت فعلياً)
- ✅ الدفع: ffc48e4 → 0226c9e → 6583d6a (تحقق GitHub API: remote HEAD = local HEAD)

---
Task ID: dedup-cdd-title-ddmmyyyy-dates
Agent: main
Task: حذف تكرار كلمات عنوان العقد (CDD) + توحيد حقول التاريخ DD/MM/YYYY في الموقع

Work Log:
- شخّص التكرار: شارة «عقد عمل محدد المدة (CDD)» في الترويسة الموحدة + عنوان القالب نفسه تحتها = العنوان مرتين، ورقم العقد مرتين (الترويسة + صف القالب)
- راجع لقطة المستخدم (نهاية الصفحة 1 / بداية الصفحة 2 + تالِي:) — هذه العلامات غير موجودة في المستودع إطلاقاً (بحث شامل + git log -S عبر التاريخ كله)؛ التكرار المتاح فعلياً في الكود = الشارة + عنوان القالب
- إزالة شارة العنوان وإخفاء رقم الوثيقة من الترويسة لوثائق CDD (القالب يحمل العنوان والرقم مرة واحدة): unifiedHeaderFor(..., showReportNumber) + unifiedReportHeaderHTML opts.showReportNumber
- إصلاح تسرّب نصي ظاهر «:v2-->» في عرض العقد المحفوظ: استبدال CDD_OFFICIAL_MARKER البادئي بregex يزيل التعليق كاملاً /<!--CDD-OFFICIAL[^>]*-->/
- مكوّن جديد src/components/ui/date-input-dmy.tsx: حقل تاريخ DD/MM/YYYY بقيمة ISO داخلية — قناع كتابة تدريجي، لصق ISO، تحقق صحة حقيقي، min/max، aria-invalid، مزامنة صامتة بلا قيم ناقصة
- تطبيقه على 5 حقول: تاريخ الميلاد + تاريخ صدور البطاقة (إضافة/تعديل عامل) + تجديد العقد (مع min=اليوم) + تاريخا العقد (إنشاء)
- توحيد عرض التواريخ: contracts-shared formatDate → DD/MM/YYYY (كانت yyyy/mm/dd) + contract-variables formatDateYMD → formatDateDMY بصيغة DD/MM/YYYY (تواريخ العقود المولدة: الميلاد/البداية/النهاية/التحرير) + عينة المعاينة 15/01/1990
- اكتشاف أثناء التشخيص: أداة الطرفية تلتهم التسلسل [h في المُخرجات فقط (not الملفات) — تحقق بـ TS AST أن new Blob([html]) سليمة أصلًا؛ لا خطأ فعلية
- تحقق متصفح كامل: كتابة 15031990 → يعرض 15/03/1990، حوار العامل بالتواريخ الجديدة، معاينة الإنشاء بلا شارة وبعنوان واحد، معاينة الأرشيف بلا تكرار وبلا تسرّب تعليق، خلايا الجدول 22/09/2026 بصيغة يوم/شهر/سنة
- تحقق API: POST /api/employees → 201 (مع birthDate وnationalIdIssueDate) ثم DELETE → 200 (تنظيف سجل الاختبار) — «فشل الحفظ» في الجلسة كان بسبب موت خادم التطوير المفاجئ وليس خللاً
- tsc نظيف + lint بلا أخطاء جديدة (القديمة في scripts/* فقط)

Stage Summary:
- ✅ عنوان عقد CDD يظهر مرة واحدة (بلا شارة ترويسة مكررة) ورقم الوثيقة مرة واحدة (في صف القالب)
