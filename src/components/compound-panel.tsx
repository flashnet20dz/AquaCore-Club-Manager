...
export function CompoundPanel() {
  ...
  const handleExport = async (format: string) => {
    ...
    try {
      ...
      a.download = `AquaCore_حقوق_المركب_${year}_${month}.${ext}`;
      a.click();
      ...
      <div className="rounded-2xl border border-border/60 bg-card overflow-hidden">
        <div className="overflow-x-auto max-h-[60vh] overflow-y-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={5} className="text-center py-12"><Loader2 className="h-6 w-6 animate-spin text-teal-600 mx-auto" /></td></tr>
              ) : entries.length === 0 ? (
                <tr>
                ...
                entries.map((entry, i) => (
                  <motion.tr
                    ...
                    className={cn(
                      "border-b transition hover:bg-teal-50/50",
                  ...
                  >
                    ...
                    <td className="p-3 text-center text-xs text-gray-700">
                      {new Date(entry.date).toLocaleDateString("ar-DZ", { day: "2-digit", month: "2-digit", year: "numeric" })}
                    </td>
