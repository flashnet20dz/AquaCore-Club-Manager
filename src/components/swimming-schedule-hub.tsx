...
import {
  ...
  Clock, Waves, Sparkles, Layers, ShieldCheck, AlertCircle, X
} from "lucide-react";
...
export function SwimmingScheduleHub() {
  ...
  // 1. تبديل أيام تشغيل المسبح (السبعة) وتحديث فوري للمنظومة
  const toggleOperatingDay = async (key: number) => {
    ...
    try {
      ...
      toast.success("تم تحديث أيام تشغيل المسبح فورياً");
      invalidateSwimConfig();
    } catch {
      toast.error("تعذر حفظ يوم التشغيل");
      fetchAll();
    } finally {
      setSavingDays(false);
    }
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="rounded-2xl border border-border/70 bg-card p-4 space-y-3 shadow-sm">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          انقر على أي يوم لتغيير حالته فوراً (مفتوح للسباحة أو عطلة وصيانة). أي تعديل هنا ينعكس تلقائياً في صفحة ساعات العمل وجدول المسبح.
        </p>
                ...
                <div className="mt-2 flex items-center gap-1">
                  {isOpen ? (
                    ...
                    <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-rose-600 dark:text-rose-400">
                      <X className="h-3 w-3" /> عطلة وصيانة
                    </span>
                  )}
                </div>
              </button>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    {!allDaysOpen && (
                      <Badge variant="outline" className="text-[10px] bg-rose-500/10 text-rose-600 border-rose-500/20 py-0.5">
                        يوم مغلق للصيانة
                      </Badge>
                    )}
                  </div>
